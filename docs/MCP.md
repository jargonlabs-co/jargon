...
`import_list` / `deploy_tool` ingest a list (table, CSV, or connected HubSpot/Railway), build the requested outbound tool, **sequence everyone automatically**, and open **Tasks**. Claude then researches each contact and calls `save_research` with talk tracks (dialer), email copy, and LinkedIn notes. The in-chat UI is **Contacts** (the list), **Sequence** (cadence structure), and **Tasks** (today's work). A **Queue** tab appears for live dialers. **Inbox** is the message log, not the send path for a cadence. Extra contact fields stay as `attrs`. `start_sequence` is only needed to re-enroll or enroll a subset. `dashboardUrl` (`https://jargonlabs.co/tools/…`) is the full web tool. Schedule with `send_message` `status: queued` + `sendAt`.
